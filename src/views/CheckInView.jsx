...
export default function CheckInView({ store }) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      {/* Hero */}
      <div
        className="flex flex-col items-center px-5 pt-6 pb-12"
        style={{ background: `linear-gradient(180deg, ${color.hex}18 0%, transparent 100%)` }}
      >
        <p className="text-sm text-gray-400 tracking-wide mb-5">
          {format(new Date(), 'yyyy年M月d日 EEEE', { locale: zhCN })}
        </p>

        {streak.current > 1 && (
          <div
            className="flex items-center gap-1.5 rounded-full px-4 py-1.5 mb-6 text-sm font-semibold"
        ...
        {streak.current <= 1 && <div className="mb-6" />}

        {/* Button */}
        <button
          onTouchEnd={e => { e.preventDefault(); handleCheckin() }}
          onClick={handleCheckin}
          className="w-48 h-48 rounded-full flex flex-col items-center justify-center select-none transition-all duration-200 active:scale-95"
          style={{
            background: flash ? '#22c55e' : color.hex,
            boxShadow: flash
              ? '0 24px 64px rgba(34,197,94,0.38)'
              : `0 24px 64px ${color.hex}48`
          }}
        >
          <svg viewBox="0 0 24 24" className="w-14 h-14 mb-1 text-white" fill="none" stroke="currentColor" strokeWidth={2.2}>
            {flash
              ? <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
              : <path strokeLinecap="round" strokeLinejoin="round" d="M12 5v14M5 12h14" />
            }
          </svg>
          <span className="text-white font-semibold text-xl">{flash ? '已打卡！' : '打卡'}</span>
        </button>
